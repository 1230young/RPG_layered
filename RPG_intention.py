...
import json
from inference_data import load_inference_data, draw_bbox, load_inference_data_with_glyph, load_intention_output_data
...
if __name__ == "__main__":
    ...
    opt.all_config_file="test/RPG_intention_base_0.0_glyph_mixed_model_sdxl-lora-128_noise-offset_train-byt5-mapper_frominit_mix_sdxl_canva_w1-1_8ep_4x16_premiumx2_albedobaseXL.py"
    ...
    if opt.layer_data is not None:
        ...
        if opt.config_dir is not None and opt.ckpt_dir is not None:
            processed_data=load_intention_output_data(layer_data)
        else:
