...
from inference_data import load_inference_data_debug, load_inference_data
...
# group_index=[{"name":"debug",'index':[58,24,66,91,29,50]}]
group_index=[{"name":"debug",'index':[58,24,66]}]
...
data_layers=load_inference_data_debug()
...
for group in group_index:
    ...
    for index in group["index"]:
        ...
        while data_layers[layer_index]['index']==index or data_layers[layer_index]['index'].split("_")[0]==str(index):
            
            folder='yuyang/code/RPG/mulit_layer_glyph_new_model_debug_small_objects_layer_ratio_base_0.5'
            img_name = f"/{data_layers[layer_index]['index']}.png"
            print(f"""
                    <div class="column">
                        <img src="{file_prefix + folder + img_name + file_sufix}" alt="image" style="max-width: 100%;">
                        <p>{img_name}</p>
                    </div>""")
            folder='yuyang/code/RPG/mulit_layer_glyph_new_model_debug_small_objects_layer_ratio_base_0.5_bbox'
            img_name = f"/{data_layers[layer_index]['index']}.png"
            print(f"""
                    <div class="column">
                        <img src="{file_prefix + folder + img_name + file_sufix}" alt="image" style="max-width: 100%;">
            ...
            if count>=3:
                ...
                print('<div class="row">')
                count=0
            if layer_index>=len(data_layers)-1:
                break
