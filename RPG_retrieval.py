...
import json
from inference_data import load_inference_data, draw_bbox, load_inference_data_with_glyph, load_retrieval
...
if __name__ == "__main__":
    ...
    if opt.layer_data is not None:
        ...
        processed_data=load_retrieval(layer_data, start_index=14)
    ...
    if demo:
        ...
        target_dir="/pyy/openseg_blob/yuyang/code/RPG"
        os.makedirs(f"{target_dir}/{directory}", exist_ok=True)
        ...
        for data in processed_data:
            ...
            for n,item in enumerate(data):
                ...
                for i in range(len(image)):
                    if "support_index" in item.keys():
                        file_name=f"{index}_{item['support_index']}.png"
                    else:
                        file_name = f"{index}_{n}.png"
                    image[i].save(f"{target_dir}/{directory}/{file_name}")
                    try:
                        draw_bbox(file_name.replace(".png",""),bboxes,f"{target_dir}/{directory}",f"{target_dir}/{directory}_bbox")
                    except:
                        pass
