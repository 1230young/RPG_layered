...
import json
from inference_data import load_inference_data, draw_bbox, load_inference_data_with_glyph, load_inference_data_debug, load_customized_debug,load_customized_debug2
...
if __name__ == "__main__":
    ...
    opt.all_config_file="test/RPG_base_0.0_glyph_model_new_debug_small_objects.py"
    ...
    if opt.layer_data is not None:
        ...
        if opt.config_dir is not None and opt.ckpt_dir is not None:
            processed_data=load_customized_debug2()
        else:
        ...
        # for cfg in [18]:
            ...
            for n,user_prompt in enumerate(user_prompts):
                ...
                for i in range(len(image)):
                    ...
                    image[i].save(f"{target_dir}/{path}")
                try:
                    draw_bbox(index_list[n],bbox,f"{target_dir}/{directory}",f"{target_dir}/{directory}_bbox")
                except:
                    pass
